...
async function init(){
  ...
  $("exportJsonBtn").onclick=()=>downloadBlob(
  ...
  window.addEventListener("online",()=>{
    syncFromCloud().catch(()=>setStatus("Offline cache"));
  });
...
async function enterApp(){
  ...
  try{
    ...
    if(!cloud.workouts.length&&appData.workouts.length){
      ...
      if(yes){
        await replaceCloudData(appData,currentUser.id);
...
function addExerciseRow(v=["","","","",""]){
  ...
  row.innerHTML=`
    <div>
      ...
      <input class="exercise-name" value="${esc(v[0])}">
    </div>

    <div>
      <label>Weight kg</label>
      <input class="exercise-weight" type="number" step="0.5" value="${esc(v[1])}">
    </div>

    <div>
      <label>Sets</label>
      <input class="exercise-sets" type="number" value="${esc(v[2])}">
    </div>

    <div>
      <label>Reps</label>
      <input class="exercise-reps" type="number" value="${esc(v[3])}">
    </div>

    <div>
      <label>RPE</label>
      <input class="exercise-rpe" type="number" min="1" max="10" step="0.5" value="${esc(v[4])}">
    </div>
...
function getAllExerciseRecords(){
  return appData.workouts.flatMap(w=>
    w.exercises.map(e=>({
      ...e,
      date:w.date,
      session:w.session
...
function getRecommendedExercises(){
  return defaultExercises.map(def=>{
    ...
    if(last.rpe>=9){
      weight=Math.max(
        0,
        last.weight-getWeightStep(last.weight)
      );
    }
    else if(last.rpe<7&&last.reps>=12){
      weight=last.weight+getWeightStep(last.weight);
    }

    reps=Math.min(reps,12);

    return [name,weight,sets,reps,rpe];
  });
}
...
function loadRecommendedWorkout(){
  ...
  $("sessionName").value=
    "Session "+(appData.workouts.length+1);
}
...
async function saveWorkout(){
  ...
  .map(row=>({
    ...
    weight:Number(row.querySelector(".exercise-weight").value)||0,
    sets:Number(row.querySelector(".exercise-sets").value)||0,
    reps:Number(row.querySelector(".exercise-reps").value)||0,
    rpe:Number(row.querySelector(".exercise-rpe").value)||0
  }))
  .filter(e=>e.name);
  ...
  const cardio={
    ...
    distance:Number($("cardioDistance").value)||0,
    speed:Number($("cardioSpeed").value)||0,
    incline:Number($("cardioIncline").value)||0,
    avgHR:Number($("cardioAverageHR").value)||0,
    peakHR:Number($("cardioPeakHR").value)||0,
    rpe:Number($("cardioRPE").value)||0,
    calories:Number($("cardioCalories").value)||0,
    recovery:Number($("hrRecovery").value)||0
  };
  ...
  const workout={
    ...
    session:$("sessionName").value||
      "Workout "+(appData.workouts.length+1),
    preHR:Number($("preWorkoutHR").value)||0,
    exercises,
    cardio
  };
  ...
  try{
    await saveWorkoutToCloud(workout,currentUser.id);
    setStatus("Synced",true);
    await syncFromCloud();
...
function renderDashboard(){
  const ws=appData.workouts;

  const total=ws.reduce(
    (s,w)=>s+(w.cardio?.minutes||0),
    0
  );

  const last=ws
    .filter(w=>w.cardio?.minutes>0)
    .at(-1);

  $("dashboardMetrics").innerHTML=`
    <div class="metric">
      <div class="metric-title">Total Workouts</div>
      <div class="metric-value">${ws.length}</div>
    </div>

    <div class="metric">
      <div class="metric-title">Cardio Minutes</div>
      <div class="metric-value">${Math.round(total)}</div>
    </div>

    <div class="metric">
      <div class="metric-title">Last Avg HR</div>
      <div class="metric-value">${last?.cardio?.avgHR||"—"}</div>
    </div>

    <div class="metric">
      <div class="metric-title">Last Cardio RPE</div>
      <div class="metric-value">${last?.cardio?.rpe||"—"}</div>
    </div>
  `;
    ...
    s.length
      ? s.map(w=>`
          <div class="progress-box">
            <strong>${esc(w.date)}</strong>
            — ${esc(w.session)}
            <br>
            <span class="small">
              ${w.exercises.length} strength exercises
              ${
                w.cardio?.minutes
                  ? ` • ${esc(w.cardio.type)}
...
function renderCardioBenchmarkStatus(){
  const s=appData.workouts.filter(w=>{
    const c=w.cardio;

    return c &&
      c.type==="Treadmill" &&
      Math.abs(c.speed-5)<.11 &&
      Math.abs(c.incline-5)<.6 &&
      c.avgHR>0;
  });

  if(!s.length){
    $("cardioBenchmarkStatus").innerHTML=
      "<p class='muted'>No standardized 5 kph / 5% treadmill sessions logged yet.</p>";
    return;
  }
  ...
  $("cardioBenchmarkStatus").innerHTML=`
    <div class="progress-box">
      <strong>Latest benchmark: ${l.cardio.avgHR} bpm</strong>
      <p>${msg}</p>
    </div>
  `;
}
...
function renderHistory(){
  const ws=[...appData.workouts].reverse();
    ...
    ws.length
      ? `
        <table>
          ...
          <tbody>
            ${ws.map((w,ri)=>{
              const i=appData.workouts.length-1-ri;
                ...
                <tr>
                  ...
                  <td>
                    ${
                      w.cardio?.minutes
                        ? `
                          ${esc(w.cardio.type)}<br>
                          ${w.cardio.minutes} min<br>
                          HR: ${w.cardio.avgHR||"—"}<br>
                          RPE: ${w.cardio.rpe||"—"}
                        `
                        : "—"
                    }
...
async function deleteWorkout(i){
  if(!confirm("Delete this workout from the cloud?"))return;

  const local=appData.workouts[i];

  try{
    if(String(local.id).startsWith("local-")){
      appData.workouts.splice(i,1);
    }else{
        ...
        <table>
          ...
          <tbody>
            ${s.map(w=>`
              <tr>
                <td>${esc(w.date)}</td>
                <td>${w.cardio.minutes} min</td>
                <td>${w.cardio.avgHR}</td>
                <td>${w.cardio.peakHR||"—"}</td>
                <td>${w.cardio.rpe||"—"}</td>
              </tr>
            `).join("")}
          </tbody>
...
async function deleteAllCloudData(){
  ...
  try{
    ...
    alert("All workout data deleted.");
